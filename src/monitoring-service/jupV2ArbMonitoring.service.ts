...
@Injectable()
export class JupV2ArbMonitoringService implements OnModuleInit, OnModuleDestroy {
  private readonly twitterNotificationSink: NotificationSink<TwitterNotification> =
    new TwitterNotificationsSink();

  private readonly logger = new Logger(JupV2ArbMonitoringService.name);
    ...
    return [
      ...jupArbTrades.arbTrades.map(
        (it) => {
          ...
          notifMsg += `\nhttps://solana.fm/tx/${it.txSignature}`;
